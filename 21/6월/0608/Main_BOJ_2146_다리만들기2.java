...
public class Main_BOJ_2146_다리만들기2 {
	static int N, bridge;
	static int[][] map;
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) {
				if (map[i][j] != 0) {
					makeBridge(i, j, new boolean[N][N]);
				}
			}
		}
	...
	private static void makeBridge(int i, int j, boolean[][] visited) {
			...
			while (--size >= 0) {
				...
				for (int k = 0; k < 4; k++) {
					...
					if (map[nx][ny] == islandNum)
						...
						if(connect < bridge) bridge = connect;
						return;
					}
				}
