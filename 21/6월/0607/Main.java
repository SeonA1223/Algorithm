...
// 아래 hunt메소드를 작성해서 제출하세요.
	...
	private static int seoul11_7_twoTimes = 2;
    private static int seoul11_7_result = 0;
    ...
    static Opponent Me=new Opponent(){
        public int hunt(int opp, int turn, int opp_prev, int opp_last_pattern[][]) {
           ...
           if(opp_last_pattern[0][0] == 0 && opp_last_pattern[0][1] == 1 && opp_last_pattern[0][2] == 2) check = 1;
           if(opp_last_pattern[0][0] == 1 && opp_last_pattern[0][1] == 2 && opp_last_pattern[0][2] == 0) check = 2;
           if(opp_last_pattern[0][0] == 2 && opp_last_pattern[0][1] == 0 && opp_last_pattern[0][2] == 1) check = 3;
           if(opp_last_pattern[0][0] == 2 && opp_last_pattern[0][1] == 1 && opp_last_pattern[0][2] == 0) check = 4;
           if(opp_last_pattern[0][0] == 1 && opp_last_pattern[0][1] == 0 && opp_last_pattern[0][2] == 2) check = 5;
           if(opp_last_pattern[0][0] == 0 && opp_last_pattern[0][1] == 2 && opp_last_pattern[0][2] == 1) check = 6;
            ...
            if(check != 0) {
               ...
               if(opp_prev == DEER) {
            	   seoul11_7_result = DEER;
                 }
                 if(seoul11_7_result == SNAKE) seoul11_7_result = DEER;
                 if(seoul11_7_result == DEER && (opp_prev == RABBIT || opp_prev == SNAKE)) {
                    if(seoul11_7_twoTimes == 2) {
                    	seoul11_7_result = DEER;
                    }else {
                    	seoul11_7_result = SNAKE;
                    }
                    seoul11_7_twoTimes--;
                    if(seoul11_7_twoTimes <= 0) seoul11_7_twoTimes = 2;
                 }
                 if(seoul11_7_result == RABBIT && opp_prev == RABBIT) {
                	 seoul11_7_result = DEER;
                 }
                 if(seoul11_7_result == RABBIT && opp_prev == SNAKE) { //배신한 구간, 한번의 관용을 베풀기
                    if(seoul11_7_twoTimes == 2) {
                    	seoul11_7_result = DEER;
                    }else {
                    	seoul11_7_result = SNAKE;
	...
	private static int seoul11_7_threeTimes = 3;
