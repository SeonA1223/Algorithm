...
public class Main_BOJ_19238_스타트택시 {
	...
	public static class Node {
		...
		public int[] getDestination(int sx, int sy) {
			if (this.sx == sx && this.sy == sy) {
				return new int[] { this.ex, this.ey };
			} else {
				return null;
			}
		}
...
//	static Map<int[], int[]> customers;
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 1; i <= M; i++) {
			...
			customers.add(new Node(sx, sy, ex, ey));
			map[sx][sy] = i;
		}
	...
	private static int[] findDestination() {
		int[] des;
		for (int i = 0; i < M; i++) {
			des = customers.get(i).getDestination(aCustomer[0], aCustomer[1]);
			if (des != null)
				return des;
		}
		return null;
